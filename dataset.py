...
class Dataset(Dataset):
    def __init__(self, dataset_path,input_shape, sequence_length, training, pose_path):
    ...
    def __getitem__(self, index):
        ...
        pose = np.load(os.path.join(self.pose_path,target,sequence_path.split('/')[-1])+'.npy')
        pose = self._transform(pose,12)
        pose = torch.tensor(pose)
        return image_sequence, pose, self.label_index[target]
