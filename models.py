...
class ConvLSTM(nn.Module):
    def __init__(
        self, num_classes, latent_dim=512, lstm_layers=1, hidden_dim=1024, bidirectional=True, attention=True, latent_dim2=24, hidden_dim2=10,
    ):
        super(ConvLSTM, self).__init__()
        self.encoder = Encoder(latent_dim)
        self.lstm = LSTM(latent_dim, lstm_layers, hidden_dim, bidirectional)
        self.lstm2 = LSTM(latent_dim2, lstm_layers, hidden_dim2, False)
        self.output_layers = nn.Sequential(
            nn.Linear(2 * hidden_dim + hidden_dim2 if bidirectional else hidden_dim + hidden_dim2, hidden_dim),
    ...
    def forward(self, x1, x2):
        ...
        x2 = self.input_layers(x2)
        x2 = x2.view(batch_size2, seq_length2, -1)
        x2 = self.lstm2(x2)
        x2 = x2[:, -1]
        return self.output_layers(torch.cat((x1, x2), dim=1))
